...
> their original identifier — `python -m atf_graphrag` — and a handful of
> environment variables retain their original prefix (`ATF_PROFILE`,
> `ATF_PARSER`, `ATF_API_TOKEN`, and so on). These are literal, unchangeable
...
- **Agentic retrieval pipeline** — query understanding → corpus selection →
  ...
  (`atf_graphrag/retrieval/pipeline.py`).
- **Vector + BM25 hybrid lane** — dense embeddings fused with lexical BM25 for
  ...
  (`atf_graphrag/retrieval/bm25.py`).
- **Graph lane** — BFS subgraph expansion (default) or **personalized PageRank
  ...
  (`atf_graphrag/retrieval/graph_retriever.py`; PPR gated behind
...
- **Deterministic table-row lane** — cell-level lookup with contiguity-aware
  ...
  (`atf_graphrag/retrieval/table_lookup.py`).
- **Text-to-SQL lane** — SELECT-only, forbidden-keyword-guarded SQLite queries
  ...
  graceful fallback to RAG (`atf_graphrag/indexing/table_store.py`).
- **Numeric lane** — headline-total rescue from number-dense text and document
  summaries (`atf_graphrag/retrieval/numeric_lookup.py`).
...
- **Corrective retry** — query reformulation and one retry round on weak or
  insufficient evidence (CRAG-style; `atf_graphrag/retrieval/adaptive.py`).
...
- **Web research lane** — Tavily augmentation into a `news` corpus, idempotent by
  URL, when local evidence is thin (`atf_graphrag/retrieval/web_research.py`).
...
- Typed entity and relation extraction over an ontology, with rule- and
  LLM-based pruning (`atf_graphrag/graph/enrich.py`,
  `atf_graphrag/graph/verify.py`, `atf_graphrag/graph/pruning.py`).
...
- **Leiden community detection** (graspologic/`leidenalg`, falling back to
  ...
  corpus-wide reasoning (`atf_graphrag/graph/communities.py`).
...
- **Docling parser** (DocLayNet layout model + TableFormer table structure) and
  ...
  config or the `ATF_PARSER` environment variable; the advanced parser is the
...
- Sitemap-driven crawler with `sitemapindex` recursion, `robots.txt` respect,
  and rate limiting (`atf_graphrag/ingestion/crawler.py`).
...
- Optional **Playwright** headless-render fallback (`auto` / `always` / `never`)
  ...
  pipeline (`atf_graphrag/ingestion/browser.py`).
...
- Storage epochs that reject stale writers (`StaleWriteError`), a PID-based
  ...
  commits (`atf_graphrag/storage_epoch.py`, `atf_graphrag/storage_lock.py`).
...
- Stdlib HTTP API on `:8077`:
  ...
  Bearer auth (`ATF_API_TOKEN`) is required when the server is not bound to
  localhost.
- Module CLI and a `scripts/` toolbox for KB builds, crawling, corpus transfer,
  and evaluation:

  ```bash
  python -m atf_graphrag serve | ingest | visual | query | stats | demo
...
- Citations on every answer, a grounding-verification subagent (reported numbers
  ...
  Guardrails for managed policies (`atf_graphrag/providers/guardrail.py`).
